...
contract UniswapV3Twap {
    ...
    function getInverseConvertion(
        ...
        uint decimal;
        address Checktoken1 = tokenIn != token1 ? token1 : token0;
        uint decimal0 = IERC20Metadata(tokenIn).decimals();
        uint decimal1 = IERC20Metadata(Checktoken1).decimals();
        decimal = decimal0 == decimal1 ? 0 : (decimal0 - decimal1);
        uint amountOut = estimateAmountOut(tokenIn, amountIn, secondsAgo);
        amount = amountOut * 10**decimal;
    }
}
