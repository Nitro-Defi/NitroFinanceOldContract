...
contract SOL5 is Position, errors, BorrowLogic {
    ...
    mapping(address => uint256) public TIR;
    // mapping(uint256 => uint64) public differenceInTime_K;
    mapping(uint256 => uint64) private LasttimeStamp_k;
    ...
    mapping(uint256 => mapping(uint256 => uint64)) private LastKtimeStamp;
    ...
    function _create(
        ...
        if (_collateralFactor > 0 && _interestRate > 0 && _value > 0) {
            ...
            //token1.transferFrom(msg.sender, address(this), _value);
    ...
    function _initBorrow(
        ...
        if (
            ...
            //token1.transfer(msg.sender, _value);
            data.liquidity -= _value;
        ...
        if (
            ...
            for (uint256 i; i < borrowData.length; i++) {
                if (
                    ...
                    TransferHelper.safeTransfer(token1, msg.sender, _value);
                    //_valuetoken1.transfer(msg.sender, _value);
                }
            }
        }
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        //token0.transfer(address(swapRouter), amountIn);
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
            .ExactInputSingleParams({
                ...
                amountIn: amountIn,
                amountOutMinimum: amountMinimun,
                sqrtPriceLimitX96: 0
            });
    ...
    //     uint256 Scaledtime = SECONDS_PER_YEAR * 100;
    //     uint256 scaledInterestRate = air[msg.sender];
    //     uint256 TimeDiff = timeElapsed * borrowedValue[msg.sender];
    //     uint256 newTotalborrows = FullMath.mulDivRoundingUp(
    //         scaledInterestRate,
    //         TimeDiff,
    //         Scaledtime
    //     );
    ...
    function getValueofInvariant(
        uint256 _collateralFactor,
        uint256 _interestRate
    ) external view returns (uint256 invariantValue) {
        LOCUS memory data = getLocus[_collateralFactor][_interestRate];
        if (data.k == 0) {
            invariantValue = 1e10;
        } else {
    ...
    function getAvaliableLiquidity(
        uint256 _collateralFactor,
        uint256 _interestRate
    ) external view returns (uint256 avaliableLiquidity) {
        LOCUS memory data = getLocus[_collateralFactor][_interestRate];
